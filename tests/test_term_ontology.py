...
class TestOntologyInit(unittest.TestCase):
    ...
    def test_index(self):
        ...
        with self.assertRaises(KeyError) as err:
            a[0]
            self.assertEqual("No HPOTerm for index 0", str(err.exception))
        with self.assertRaises(KeyError) as err:
            a[2]
            self.assertEqual("No HPOTerm for index 2", str(err.exception))

        with self.assertRaises(KeyError) as err:
            a[4]
            self.assertEqual("No HPOTerm for index 4", str(err.exception))
...
class TestBasicOntologyFeatures(unittest.TestCase):
    ...
    def test_parents(self):
        ...
        self.assertIn(self.child_2_1, self.child_3.parents)
        self.assertIn(self.child_1_2, self.child_3.parents)

        assert len(self.child_2_1.parents) == 1
        self.assertEqual(self.child_2_1.parents, set([self.child_1_1]))

        for p in self.child_2_1.parents:
            self.assertIn(self.root, p.parents)

        assert len(self.child_1_1.parents) == 1
        self.assertIn(self.root, self.child_1_1.parents)

        assert len(self.child_1_2.parents) == 1
        self.assertIn(self.root, self.child_1_2.parents)

        self.assertEqual(self.root.parents, set())

    def test_children(self):
        assert len(self.root.children) == 2
        self.assertEqual(set([self.child_1_1, self.child_1_2]), self.root.children)
...
class TestOntologyTreeTraversal(unittest.TestCase):
    ...
    def test_hierarchy(self):
        self.assertEqual(self.root.hierarchy, ((self.root,),))

        self.assertEqual(self.child_1_1.hierarchy, ((self.child_1_1, self.root),))

        self.assertEqual(self.child_1_2.hierarchy, ((self.child_1_2, self.root),))

        self.assertEqual(
            self.child_2_1.hierarchy, ((self.child_2_1, self.child_1_1, self.root),)
        )
    ...
    def test_path_to_parent(self):
        ...
        self.assertEqual(str(err.exception), "HP:0013 is not a parent of HP:0041")

        with self.assertRaises(RuntimeError) as err:
            _ = self.child_1_2.shortest_path_to_parent(self.child_4)
        self.assertEqual(str(err.exception), "HP:0041 is not a parent of HP:0012")
    ...
    def test_the_impossible(self, mock_sorted):
        ...
        self.assertEqual(
            str(err.exception), "Unable to determine path to parent term Test root"
        )

    def test_path_to_other(self):
        path = self.child_1_1.path_to_other(self.root)
        assert path == (1, (self.child_1_1, self.root), 1, 0)

        path = self.root.path_to_other(self.child_1_1)
        assert path == (1, (self.root, self.child_1_1), 0, 1)

        path = self.child_1_1.path_to_other(self.child_1_2)
        assert path == (2, (self.child_1_1, self.root, self.child_1_2), 1, 1)

        path = self.child_1_1.path_to_other(self.child_1_3)
        assert path == (2, (self.child_1_1, self.root, self.child_1_3), 1, 1)
        ...
        assert path == (
            4,
            (self.child_4, self.child_3, self.child_1_2, self.root, self.child_1_3),
            3,
            1,
        )
        ...
        assert path == (
            4,
            (self.child_1_3, self.root, self.child_1_2, self.child_3, self.child_4),
            1,
            3,
        )
    ...
    def test_builtins(self):
        ...
        self.assertEqual(
            repr(self.root), "HPOTerm(id='HP:0001', name='Test root', is_a=[])"
        )
...
class TestOntologyQueries(unittest.TestCase):
    ...
    def test_get_hpo_object(self):
        self.assertEqual(
            self.terms.get_hpo_object("Test child level 1-2"), self.child_1_2
        )
        self.assertEqual(self.terms.get_hpo_object("HP:00012"), self.child_1_2)
        self.assertEqual(self.terms.get_hpo_object(12), self.child_1_2)
    ...
    def test_matching(self):
        self.assertEqual(self.terms.match(self.root.name), self.root)
        self.assertEqual(self.terms.match(self.child_1_1.name), self.child_1_1)
        self.assertEqual(self.terms.match(self.child_1_2.name), self.child_1_2)
        self.assertEqual(self.terms.match(self.child_1_3.name), self.child_1_3)
        self.assertEqual(self.terms.match(self.child_1_2.name), self.child_1_2)
        self.assertEqual(self.terms.match(self.child_3.name), self.child_3)
        self.assertEqual(self.terms.match(self.child_4.name), self.child_4)
    ...
    def test_path_integration(self):
        ...
        self.assertEqual(path, (1, (self.child_1_1, self.root), 1, 0))

        path = self.terms.path("Test root", "Test child level 1-1")
        self.assertEqual(path, (1, (self.root, self.child_1_1), 0, 1))

        path = self.terms.path("Test child level 1-1", "Test child level 1-2")
        self.assertEqual(path, (2, (self.child_1_1, self.root, self.child_1_2), 1, 1))

    @patch("pyhpo.Ontology.synonym_search")
    def test_search(self, mock_syn_search):
        mock_syn_search.return_value = False

        self.assertEqual(list(self.terms.search("something")), [])
        # All terms will be searched for in synonyms
        self.assertEqual(mock_syn_search.call_count, 7)
        mock_syn_search.reset_mock()

        self.assertEqual(list(self.terms.search("Test root")), [self.root])
        # Root term will not be searched for in synonyms
        self.assertEqual(mock_syn_search.call_count, 6)
        mock_syn_search.reset_mock()

        self.assertEqual(
            list(self.terms.search("Test child level 1-1")), [self.child_1_1]
        )
    ...
    def test_synonym_match(self):
        ...
        self.assertEqual(
            self.terms.synonym_match("Test child level 1-2"), self.child_1_2
        )
        self.assertEqual(self.terms.synonym_match("another name"), self.child_1_2)
        self.assertEqual(self.terms.synonym_match("third name"), self.child_1_2)

        self.child_1_2.name = "something else"
        self.assertEqual(
            self.terms.synonym_match("Test child level 1-2"), self.child_1_1
        )
...
class TestOntologyAnnotation(unittest.TestCase):
    ...
    def test_decipher_diseases(self):
        assert (
            len(self.ontology.decipher_diseases) == 3
        ), self.ontology.decipher_diseases
