...
class TestStaticMethods(unittest.TestCase):
    def test_id_parsing(self):
        ...
        assert id_from_string("HP:0001507 ! Growth abnormality") == 1507
        assert id_from_string("HP:0000105") == 105

    def test_id_parsing_newline(self):
        assert id_from_string("HP:00001\n") == 1
        assert id_from_string("HP:0001507 ! Growth abnormality\n") == 1507
    ...
    def test_id_parsing_errors(self):
        ...
        with self.assertRaises(AttributeError) as context:
            id_from_string(12)
        self.assertEqual(
            "'int' object has no attribute 'split'", str(context.exception)
        )
...
class TestTermInit(unittest.TestCase):
    ...
    def test_init(self):
        ...
        self.assertEqual(self.term._hash, hash(self.term))
        self.assertEqual(hash(self.term), hash((self.term.index, self.term.name)))

    def test_id(self):
        self.assertEqual(hash(self.term.index), 4944)
...
class TestSingleTermAttributes(unittest.TestCase):
    ...
    def test_term(self):
        ...
        self.assertEqual(
            term.comment, "Aneurysm is considered a severe form of dilatation."
        )
        assert term.synonym == [
            "Brain aneurysm",
            "Cerebral aneurysm",
        ...
        self.assertEqual(term.parent_ids(), [2617, 9145])
        self.assertEqual(term.hierarchy, ((term,),))
        self.assertEqual(term.toJSON().keys(), {"int", "id", "name"})
        self.assertEqual(
        ...
        self.assertEqual(term.toJSON()["id"], term.id)
        self.assertEqual(term.toJSON()["int"], term.index)
        self.assertEqual(term.toJSON()["name"], term.name)
        self.assertEqual(term.toJSON(verbose=True)["definition"], term.definition)
        self.assertEqual(term.toJSON(verbose=True)["comment"], term.comment)
        self.assertEqual(term.toJSON(verbose=True)["synonym"], term.synonym)
        self.assertEqual(term.toJSON(verbose=True)["xref"], term.xref)
        self.assertEqual(term.toJSON(verbose=True)["is_a"], term._is_a)
...
class TestTermAnnotations(unittest.TestCase):
    ...
    def test_enter_genes(self):
        ...
        self.assertEqual(self.term.genes, set(["Gene1"]))

        add_gene_to_term("Gene2", self.term)
        self.assertIn("Gene1", self.term.genes)
        self.assertIn("Gene2", self.term.genes)
        self.assertEqual(len(self.term.genes), 2)

    def test_gene_parent_update(self):
        term2 = HPOTerm(id="HP:000001", name="General parent term")
        ...
        self.assertEqual(self.term.genes, set(["Gene1"]))
        self.assertEqual(term2.genes, set(["Gene1", "Gene2"]))
...
class TestTermSimilarities(unittest.TestCase):
    ...
    def test_similarity_call(self, mock_simscore):
        ...
        mock_simscore.assert_called_once_with(self.term, "mockterm", None, None)

    @patch("pyhpo.term.SimScore")
    def test_similarity_method_call(self, mock_simscore):
        self.term.similarity_score("mockterm", method="foo")
        mock_simscore.assert_called_once_with(self.term, "mockterm", None, "foo")
        ...
        mock_simscore.assert_called_once_with(self.term, "mockterm", "", "foo")

    @patch("pyhpo.term.SimScore")
    def test_similarity_kind_call(self, mock_simscore):
        self.term.similarity_score("mockterm", kind="foo")
        mock_simscore.assert_called_once_with(self.term, "mockterm", "foo", None)
    ...
    def test_similarity_kind_method_call(self, mock_simscore):
        ...
        mock_simscore.assert_called_once_with(self.term, "mockterm", "foo", "bar")
