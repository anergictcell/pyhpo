...
class OboParsingTest(unittest.TestCase):
    ...
    def test_root_term(self):
        ...
        self.assertEqual(
            a["comment"], "Root of all terms in the Human Phenotype Ontology."
        )
        ...
        self.assertEqual(term.name, "All")

    def test_full_load(self):
        data_dir = os.path.join(os.path.dirname(__file__), "../pyhpo/data")
...
class TestBasic(unittest.TestCase):
    ...
    def test_synonym_parsing(self):
        assert Converter.parse_synonym(
            ['"Abnormality of body height" EXACT layperson []'], "synonym", []
        ) == ["Abnormality of body height"]

        assert Converter.parse_synonym(
            ['"Autosomal dominant type" RELATED [HPO:skoehler]'], "synonym", []
    ...
    def test_synonym_parsing_errors(self):
        with self.assertRaises(IndexError) as context:
            Converter.parse_synonym(["synonym: Scalp hair loss"], "synonym", [])
